...
#define PTN_MAX_PTN_ENTRIES_XGA 1024
...
typedef struct
{
    ...
    uint16 wrBytes;         /** Number of bytes to write */
    uint16 rdBytes;         /** Number of bytes to read */
...
typedef enum
{
    ...
    PWM_CAPTURE_READ,
    I2C_PASSTHRU,
    PATMEM_LOAD_INIT_PRIMARY,
    PATMEM_LOAD_DATA_PRIMARY,
    PATMEM_LOAD_INIT_SECONDARY,
    PATMEM_LOAD_DATA_SECONDARY,
    PATMEM_LOAD_INIT_MASTER, // For backward comaptablity.
    PATMEM_LOAD_DATA_MASTER, // For backward comaptablity.
    PATMEM_LOAD_INIT_SLAVE,  // For backward comaptablity.
    PATMEM_LOAD_DATA_SLAVE,  // For backward comaptablity.
    BATCHFILE_NAME,
    BATCHFILE_EXECUTE,
    DELAY,
    DEBUG,
    ...
    LED_PULSE_WIDTH,
    MIN_EXPOSURE,
    LOADNGO_INIT_PRIMARY,
    LOADNGO_DATA_PRIMARY,
    LOADNGO_INIT_SECONDARY,
    LOADNGO_DATA_SECONDARY,
    LOADNGO_INIT_MASTER,    // For backward comaptablity.
    LOADNGO_DATA_MASTER,    // For backward comaptablity.
    LOADNGO_INIT_SLAVE, // For backward comaptablity.
    LOADNGO_DATA_SLAVE, // For backward comaptablity.
    LOADNGO_SRAM,
    READ_WRITE_SRAM,
    BL_STATUS,
    BL_SPL_MODE,
    ...
    BL_CALC_CHKSUM,
    BL_PROG_MODE,
    BL_PRIMARY_SECONDARY,
    BL_MASTER_SLAVE,    // For backward comaptablity.
...
static unsigned char InputBuffer[HID_MESSAGE_MAX_SIZE + sizeof(((hidMessageStruct*)0)->head)];

static CmdFormat CmdList[] =
{
    { "SOURCE_SEL"              , 0x00, 0x1A00, 0x01, 0x01, 0 },
    { "PIXEL_FORMAT"            , 0x02, 0x1A02, 0x01, 0x01, 0 },
    { "CLK_SEL"                 , 0x03, 0x1A03, 0x01, 0x01, 0 },
    { "CHANNEL_SWAP"            , 0x04, 0x1A37, 0x01, 0x01, 0 },
    { "FPD_MODE"                , 0x05, 0x1A04, 0x01, 0x01, 0 },
    { "POWER_CONTROL"           , 0x07, 0x0200, 0x01, 0x01, 0 },
    { "FLIP_LONG"               , 0x08, 0x1008, 0x01, 0x01, 0 },
    { "FLIP_SHORT"              , 0x09, 0x1009, 0x01, 0x01, 0 },
    { "TPG_SEL"                 , 0x0A, 0x1203, 0x01, 0x01, 0 },
    { "PWM_INVERT"              , 0x0B, 0x1A05, 0x01, 0x01, 0 },
    { "DSTN_ASIC"               , 0x0E, 0x0202, 0x01, 0x01, 0 },
    { "LED_ENABLE"              , 0x10, 0x1A07, 0x01, 0x01, 0 },
    { "GET_VERSION"             , 0x11, 0x0205, 0x00, 0x10, 0 },
    { "SW_RESET"                , 0x13, 0x0802, 0x01, 0x01, 0 },
    { "DMD_PARK"                , 0x14, 0x0609, 0x01, 0x01, 0 },
    { "DLPA200_FAULT"           , 0x19, 0x1A48, 0x01, 0x01, 0 },
    { "DMD_CON_STATUS"          , 0x18, 0x1A49, 0x01, 0x01, 0 },
    { "STATUS_HW"               , 0x20, 0x1A0A, 0x00, 0x01, 0 },
    { "STATUS_SYS"              , 0x21, 0x1A0B, 0x00, 0x01, 0 },
    { "STATUS_MAIN"             , 0x22, 0x1A0C, 0x00, 0x01, 0 },
    { "PWM_ENABLE"              , 0x40, 0x1A10, 0x01, 0x01, 0 },
    { "PWM_SETUP"               , 0x41, 0x1A11, 0x06, 0x06, 0 },
    { "PWM_CAPTURE_CONFIG"      , 0x43, 0x1A12, 0x05, 0x05, 0 },
    { "GPIO_CONFIG"             , 0x44, 0x1A38, 0x02, 0x01, 0 },
    { "LED_CURRENT"             , 0x4B, 0x0B01, 0x03, 0x03, 0 },
    { "LED_FREQUENCY"           , 0x4C, 0x0B02, 0x04, 0x04, 0 },
    { "DISP_CONFIG"             , 0x7E, 0x1000, 0x10, 0x10, 0 },
    { "DISP_MODE"               , 0x69, 0x1A1B, 0x01, 0x01, 0 },
    { "TRIG_OUT1_CTL"           , 0x6A, 0x1A1D, 0x05, 0x05, 0 },
    { "TRIG_OUT2_CTL"           , 0x6B, 0x1A1E, 0x05, 0x05, 0 },
    { "RED_LED_ENABLE_DLY"      , 0x6C, 0x1A1F, 0x05, 0x05, 0 },
    { "GREEN_LED_ENABLE_DLY"    , 0x6D, 0x1A20, 0x05, 0x05, 0 },
    { "BLUE_LED_ENABLE_DLY"     , 0x6E, 0x1A21, 0x05, 0x05, 0 },
    { "PAT_START_STOP"          , 0x65, 0x1A24, 0x01, 0x00, 0 },
    { "TRIG_IN1_CTL"            , 0x79, 0x1A35, 0x03, 0x03, 0 },
    { "TRIG_IN2_CTL"            , 0x7A, 0x1A36, 0x01, 0x01, 0 },
    { "INVERT_DATA"             , 0x74, 0x1A30, 0x01, 0x01, 0 },
    { "PAT_CONFIG"              , 0x75, 0x1A31, 0x06, 0x06, 0 },
    { "MBOX_ADDRESS"            , 0x76, 0x1A32, 0x02, 0x00, 0 },
    { "MBOX_CONTROL"            , 0x77, 0x1A33, 0x01, 0x00, 0 },
    { "MBOX_DATA"               , 0x78, 0x1A34, 0x0C, 0x00, 0 },
    { "SPLASH_LOAD"             , 0x7F, 0x1A39, 0x01, 0x01, 0 },
    { "GPCLK_CONFIG"            , 0x48, 0x0807, 0x03, 0x04, 0 },
    { "TPG_COLOR"               , 0x1A, 0x1204, 0x0C, 0x0C, 0 },
    { "PWM_CAPTURE_READ"        , 0x4E, 0x1A13, 0x05, 0x05, 0 },
    { "I2C_PASSTHRU"            , 0x4F, 0x1A4F, 0x05, 0x07, 2 },
    { "PATMEM_LOAD_INIT_PRIMARY" , 0x2A, 0x1A2A, 0x06, 0x00, 0 },
    { "PATMEM_LOAD_DATA_PRIMARY" , 0x2B, 0x1A2B, 0x00, 0x00, 2 },
    { "PATMEM_LOAD_INIT_SECONDARY", 0x2C, 0x1A2C, 0x06, 0x00, 0 },
    { "PATMEM_LOAD_DATA_SECONDARY", 0x2D, 0x1A2D, 0x00, 0x00, 2 },
    { "PATMEM_LOAD_INIT_MASTER" , 0x2A, 0x1A2A, 0x06, 0x00, 0 },
    { "PATMEM_LOAD_DATA_MASTER" , 0x2B, 0x1A2B, 0x00, 0x00, 2 },
    { "PATMEM_LOAD_INIT_SLAVE"  , 0x2C, 0x1A2C, 0x06, 0x00, 0 },
    { "PATMEM_LOAD_DATA_SLAVE"  , 0x2D, 0x1A2D, 0x00, 0x00, 2 },
    { "BATCHFILE_NAME"          , 0x3A, 0x1A14, 0x01, 0x10, 0 },
    { "BATCHFILE_EXECUTE"       , 0x3B, 0x1A15, 0x01, 0x00, 0 },
    { "DELAY"                   , 0x3C, 0x1A16, 0x04, 0x00, 0 },
    { "DEBUG"                   , 0x5B, 0x1A5B, 0x05, 0x05, 0 },
    { "I2C_CONFIG"              , 0x45, 0x1A4E, 0x05, 0x00, 0 },
    { "CURTAIN_COLOR"           , 0x06, 0x1100, 0x06, 0x06, 0 },
    { "VIDEO_CONT_SEL"          , 0x0C, 0x1A01, 0x01, 0x01, 0 },
    { "READ_ERROR_CODE"         , 0x32, 0x0100, 0x01, 0x01, 0 },
    { "READ_ERROR_MSG"          , 0x33, 0x0101, 0x80, 0x80, 0 },
    { "READ_FRMW_VERSION"       , 0x12, 0x0206, 0x21, 0x21, 0 },
    { "DMD_BLOCKS"              , 0x60, 0x1A40, 0x02, 0x02, 0 },
    { "DMD_IDLE"                , 0x0D, 0x0201, 0x01, 0x01, 0 },
    { "LED_PULSE_WIDTH"         , 0x64, 0x1A43, 0x04, 0x04, 0 },
    { "MIN_EXPOSURE"            , 0x63, 0x1A42, 0x02, 0x20, 0 },
    { "LOADNGO_INIT_PRIMARY"    , 0x14, 0x1A43, 0x04, 0x00, 0 },
    { "LOADNGO_DATA_PRIMARY"    , 0x15, 0x1A44, 0x02, 0x00, 2 },
    { "LOADNGO_INIT_SECONDARY"  , 0x16, 0x1A45, 0x04, 0x00, 0 },
    { "LOADNGO_DATA_SECONDARY"  , 0x17, 0x1A46, 0x02, 0x00, 2 },
    { "LOADNGO_INIT_MASTER"     , 0x14, 0x1A43, 0x04, 0x00, 0 },
    { "LOADNGO_DATA_MASTER"     , 0x15, 0x1A44, 0x02, 0x00, 2 },
    { "LOADNGO_INIT_SLAVE"      , 0x16, 0x1A45, 0x04, 0x00, 0 },
    { "LOADNGO_DATA_SLAVE"      , 0x17, 0x1A46, 0x02, 0x00, 2 },
    { "LOADNGO_SRAM"            , 0x18, 0x1A47, 0x01, 0x00, 0 },
    { "READ_WRITE_SRAM"         , 0x5D, 0x1A70, 0x09, 0x00, 0 },
    { "BL_STATUS"               , 0x01, 0x0000, 0x00, 0x00, 0 },
    { "BL_SPL_MODE"             , 0x23, 0x0023, 0x01, 0x00, 0 },
    { "BL_GET_FLASH_PRESENT"    , 0x0B, 0x0015, 0x01, 0x00, 0 },
    { "BL_GET_MANID"            , 0x0C, 0x0015, 0x01, 0x00, 0 },
    { "BL_GET_DEVID"            , 0x0D, 0x0015, 0x01, 0x00, 0 },
    { "BL_GET_CHKSUM"           , 0x00, 0x0015, 0x01, 0x00, 0 },
    { "BL_SET_SECTADDR"         , 0x29, 0x0029, 0x04, 0x00, 0 },
    { "BL_SECT_ERASE"           , 0x28, 0x0028, 0x00, 0x00, 0 },
    { "BL_SET_DNLDSIZE"         , 0x2C, 0x002C, 0x04, 0x00, 0 },
    { "BL_DNLD_DATA"            , 0x25, 0x0025, 0x00, 0x00, 0 },
    { "BL_FLASH_TYPE"           , 0x2F, 0x002F, 0x01, 0x00, 0 },
    { "BL_CALC_CHKSUM"          , 0x26, 0x0026, 0x00, 0x00, 0 },
    { "BL_PROG_MODE"            , 0x30, 0x0030, 0x01, 0x00, 0 },
    { "BL_PRIMARY_SECONDARY"    , 0x31, 0x0031, 0x01, 0x00, 0 },
    { "BL_MASTER_SLAVE"         , 0x31, 0x0031, 0x01, 0x00, 0 },
    { "BL_SET_SECTADDR_4BYTE"   , 0x32, 0x0032, 0x04, 0x00, 0 },
    { "BL_SET_DNLDSIZE_4BYTE"   , 0x33, 0x0033, 0x04, 0x00, 0 },
    { "BUSY_GPIO_CONFIG"        , 0x5E, 0x1A5E, 0x01, 0x00, 0 },
    { "FLASH_TEST"              , 0x5F, 0x1A3A, 0x08, 0x08, 0 },
    { "DE_GAMMA_SET"            , 0x61, 0x1A3B, 0x02, 0x02, 0 },
    { "PARALLEL_PORT"           , 0x64, 0x1A3C, 0x12, 0x12, 0 },
    { "STANDBY_DELAY"           , 0x66, 0x1A25, 0x01, 0x01, 0 },
    { "MEMORY_RW"               , 0x53, 0x1A53, 0x09, 0x08, 0 }
};

static unsigned char seqNum=0;
static unsigned char PatLut[12][PTN_MAX_PTN_ENTRIES_XGA];
...
int LCR_FillCmdInfo(hidMessageStruct *pMsg, LCR_CMD cmd);
DIAG_CommandInfo_t DIAG_CmdInfo;
...
int LCR_Write(BOOL ackRequired)
{
    ...
    if (!ackRequired)
    {
        ret_val = USB_Write(OutputBuffer);
        DIAG_updateProjectorControl(&DIAG_CmdInfo, ret_val);
        DIAG_updateCommandLog(&DIAG_CmdInfo, OutputBuffer, InputBuffer, ret_val);
        return ret_val;
    }
 ...
 */
{
    ...
    ret_val = -1;
    DIAG_updateProjectorControl(&DIAG_CmdInfo, -1);
    DIAG_updateCommandLog(&DIAG_CmdInfo, OutputBuffer, InputBuffer, ret_val);
    return ret_val;
}
 ...
 */
{
    ...
    pMsg->head.length = CmdList[cmd].wrBytes + 2;

    LCR_FillCmdInfo(pMsg, cmd);
...
int LCR_FillCmdInfo(hidMessageStruct *pMsg, LCR_CMD cmd)
/**
 * This function is private to this file. Stores the command information with the given command code and the message structure pointer passed.
 * Used to log command information in Diagnostic Window
 *
 * @param   cmd  - I - USB command code.
 * @param   pMsg - I - Pointer to the message.
 *
 ...
 */
{
    DIAG_CmdInfo.USBCmd = CmdList[cmd].USBCMD;
    DIAG_CmdInfo.Read = pMsg->head.flags.rw;
    DIAG_CmdInfo.CmdName = CmdList[cmd].name;
    if(pMsg->head.flags.rw)
    {
        DIAG_CmdInfo.wrBytes = sizeof(pMsg->head)+sizeof(pMsg->text.cmd) + CmdList[cmd].wrBytes;
        DIAG_CmdInfo.rdBytes = sizeof(pMsg->head) + sizeof(pMsg->text.cmd) + CmdList[cmd].rdBytes;
    }
    else
    {
        DIAG_CmdInfo.wrBytes = sizeof(pMsg->head) + sizeof(pMsg->text.cmd) + CmdList[cmd].wrBytes + 2;
        DIAG_CmdInfo.rdBytes = sizeof(pMsg->head) + sizeof(pMsg->text.cmd) + CmdList[cmd].rdBytes;
    }
...
int LCR_SetDestinationController(unsigned int DstnASIC)
{
    hidMessageStruct msg;

    msg.text.data[2] = DstnASIC;

    LCR_PrepWriteCmd(&msg, DSTN_ASIC);

    return LCR_SendMsg(&msg);
}

int LCR_GetDestinationController(unsigned int *pDstnASIC)
{
    hidMessageStruct msg;

    LCR_PrepReadCmd(DSTN_ASIC);
    if(LCR_Read() > 0)
    {
        memcpy(&msg, InputBuffer, 65);

        *pDstnASIC =   msg.text.data[0];
        return 0;
    }
    return -1;
}
 ...
 */
{
    ...
    LCR_PrepWriteCmd(&msg, BL_PRIMARY_SECONDARY);
 ...
 */
{
    ...
    CmdList[BL_DNLD_DATA].wrBytes = dataLen;
...
int LCR_GetStatus(unsigned char *pHWStatus, unsigned char *pSysStatus, unsigned char *pMainStatus, unsigned char *pDLPA200Status, unsigned char *pDMDConnectionStatus)
/**
 ...
 */
{
    ...
    unsigned int DestinationASIC = 0;
    LCR_GetDestinationController(&DestinationASIC);
    printf("DestinationASIC: %u\n", DestinationASIC);
    if(DestinationASIC != 2)
    {
        printf("Reading DLPA200_FAULT...\n");
        LCR_PrepReadCmd(DLPA200_FAULT);
        readResult = LCR_Read();
        printf("  Result: %d\n", readResult);
        if(readResult > 0)
        {
            memcpy(&msg, InputBuffer, 65);
            *pDLPA200Status = msg.text.data[0];
            printf("  DLPA200 Status: 0x%02X\n", *pDLPA200Status);
        }
        else
        {
            printf("  FAILED!\n");
        ...
        printf("Reading DMD_CON_STATUS...\n");
        LCR_PrepReadCmd(DMD_CON_STATUS);
        readResult = LCR_Read();
        printf("  Result: %d\n", readResult);
        if(readResult > 0)
        {
            memcpy(&msg, InputBuffer, 65);
            *pDMDConnectionStatus = msg.text.data[0];
            printf("  DMD Connection: 0x%02X\n", *pDMDConnectionStatus);
        }
        else
        {
            printf("  FAILED!\n");
    ...
    {
        *pDLPA200Status = 0x0;
        *pDMDConnectionStatus = 0x0;
        printf("Skipping DLPA200/DMD (ASIC=2)\n");
    }
    printf("GetStatus complete.\n");
    return 0;
}
 ...
 */
{
    ...
    CmdList[MBOX_ADDRESS].wrBytes = size ;
    LCR_PrepWriteCmd(&msg, MBOX_ADDRESS);
...
int LCR_SetI2CConfigure(unsigned int port, unsigned int addm, unsigned int clk)
/**
 * (I2C: 0x4E)
 ...
 */
{
    ...
    msg.head.length = CmdList[I2C_CONFIG].wrBytes + 2;
...
int LCR_GetI2CConfigure(unsigned int port, unsigned int *addm, unsigned int *clk)
/**
 * (I2C: 0x4E)
 * (USB: CMD2: 0x1A, CMD3: 0x4E)
 *
 ...
 */
{
    ...
    LCR_PrepReadCmdWithParam(I2C_CONFIG, (unsigned char)port);
    if(LCR_Read() > 0)
    {
        memcpy(&msg, InputBuffer, 65);
        *addm = InputBuffer[4];
        *clk = MAKE_WORD32(InputBuffer[8], InputBuffer[7], InputBuffer[6], InputBuffer[5]);
        return 0;
    }
    return -1;
}
 ...
 */
{
    ...
    msg.head.length = CmdList[I2C_PASSTHRU].wrBytes + nwbytes + 2;
...
int LCR_InitPatternMemLoad(BOOL master, unsigned short imageNum, unsigned int size)
{
    ...
    if (master)
        LCR_PrepWriteCmd(&msg, PATMEM_LOAD_INIT_PRIMARY);
    else
        LCR_PrepWriteCmd(&msg, PATMEM_LOAD_INIT_SECONDARY);
...
int LCR_pattenMemLoad(BOOL master, unsigned char *pByteArray, int size)
{
    ...
    if (master)
    {
        CmdList[PATMEM_LOAD_DATA_PRIMARY].wrBytes = dataLen + 2;
        LCR_PrepWriteCmd(&msg, PATMEM_LOAD_DATA_PRIMARY);
    }
    else
    {
        CmdList[PATMEM_LOAD_DATA_SECONDARY].wrBytes = dataLen + 2;
        LCR_PrepWriteCmd(&msg, PATMEM_LOAD_DATA_SECONDARY);
    }
 ...
 */
{
    ...
    if(LCR_Read() > 0)
    {
        memcpy(&msg, InputBuffer, USB_MAX_PACKET_SIZE + 1);

        for(i=0;i<USB_MAX_PACKET_SIZE - 4;i++)
        {
...
int LCR_LoadNGo_Init(uint32 size)
{
    ...
    LCR_PrepWriteCmd(&msg, LOADNGO_INIT_PRIMARY);
...
/*
int LCR_SetDLPA200FaultType(BOOL DLPA200_Therm, BOOL DLPA200_Volt, BOOL DLPA200_Curr)
 * Use this API for DLPA200 interrupt simulation
 * @param DLPA200_Therm - O - check for DLPA200 thermal fault
 * @param DLPA200_Volt - O - check for DLPA200 voltage fault
 * @param DLPA200_Curr - O - check for DLPA200 current fault
 *
 * @return  0 = PASS    <BR>
 *          -1 = FAIL  <BR>
 *

{
    ...
    msg.text.data[2] = Enable;
    LCR_PrepWriteCmd(&msg, DLPA200_FAULT);

    return LCR_SendMsg(&msg);
}
*/

int LCR_GetDLPA200FaultType(BOOL *pDLPA200_Therm, BOOL *pDLPA200_Volt, BOOL *pDLPA200_Curr)
/**
 * Use this API to get DLPA200 fault
 * @param *pDLPA200_Therm - O - set if DLPA200 thermal fault has occured
 * @param *pDLPA200_Volt - O - set if DLPA200 voltage has occured
 * @param *pDLPA200_Curr - O - set if DLPA200 current fault has occured
 *
 * @return  0 = PASS    <BR>
 *          -1 = FAIL  <BR>
 *
 */
{
    ...
    if(LCR_Read() > 0)
    {
        ...
        if(msg.text.data[0] & BIT3)
            ...
            *pDLPA200_Therm = FALSE;

        if(msg.text.data[0] & BIT4)
            *pDLPA200_Volt = TRUE;
        else
            *pDLPA200_Volt = FALSE;

        if(msg.text.data[0] & BIT5)
            *pDLPA200_Curr = TRUE;
        else
            *pDLPA200_Curr = FALSE;
...
int LCR_GetDMDCommErrorType(BOOL *pDMD_Comm_Error, BOOL *pDMD_DAD_Comm_Error, BOOL *pDMD_SSP_Init_Error)
{
    hidMessageStruct msg;
    ...
    if(LCR_Read() > 0)
    {
        ...
        if(msg.text.data[0] & BIT0)
            *pDMD_Comm_Error = TRUE;
        else
            *pDMD_Comm_Error = FALSE;

        if(msg.text.data[0] & BIT1)
            *pDMD_DAD_Comm_Error = TRUE;
        else
            *pDMD_DAD_Comm_Error = FALSE;

        if(msg.text.data[0] & BIT2)
            *pDMD_SSP_Init_Error = TRUE;
        else
            *pDMD_SSP_Init_Error = FALSE;
...
int LCR_GetMemory(uint32 Address, uint32 *pPriData, uint32 *pSecData, uint08 FromWhom)
/**
 * (I2C: 0x53)
 * (USB: CMD2: 0x1A, CMD3: 0x53)
 *
 * @param Address - I - Memory address to read data from
 * @param *priData - O - Data at the memory address from primary ASIC
 * @param *pSecData - O - Data at the memory address from secondary ASIC
 * @param FromWhom - I - 0 = Read data from both ASICs (8 bytes)
 *                       1 = Read data from primary ASICs only (4 bytes)
 *                       2 = Read data from secondary ASICs only (4 bytes)
 *
 * @return  >=0 = PASS    <BR>
 *          <0 = FAIL  <BR>
 *
 */
{
    LCR_PrepReadCmdWithWordParam(MEMORY_RW, Address, FromWhom);
    ...
    if(LCR_Read() > 0)
    {
        ...
        case 0:
        {
            *pPriData = MAKE_WORD32(InputBuffer[7], InputBuffer[6], InputBuffer[5], InputBuffer[4]);
            *pSecData = MAKE_WORD32(InputBuffer[11], InputBuffer[10], InputBuffer[9], InputBuffer[8]);
            break;
        }
        case 1:
        {
        ...
        case 2:
        {
            *pSecData = MAKE_WORD32(InputBuffer[7], InputBuffer[6], InputBuffer[5], InputBuffer[4]);
            break;
        }
        }
...
int LCR_SetMemory(uint32 Address, uint32 Data, uint08 ToWhom)
/**
 * (I2C: 0x53)
 * (USB: CMD2: 0x1A, CMD3: 0x53)
 *
 * @param Address - I - Memory address to to write data to
 * @param Data - I - Data to be written
 * @param ToWhom - I - Destination ASIC where to write data to
 *
 * @return  >=0 = PASS    <BR>
 *          <0 = FAIL  <BR>
 *
...
{
    hidMessageStruct msg;
    uint32 *pAddress = (uint32*)&msg.text.data[2];
    uint32 *pData = (uint32*)&msg.text.data[6];
    msg.text.data[10] = ToWhom;

    *pAddress = Address;
    *pData = Data;
    LCR_PrepWriteCmd(&msg, MEMORY_RW);
...
/**
 * (I2C: 0x5B)
 * (USB: CMD2: 0x1A, CMD3: 0x5B)
 *
 * @param *enable - O - Each major sub-system is provided with 1-bit mask. Get enabled levels.
 * @param *alloc - O - Message string allocation for debug
 * @param *used - O - Amount of string used for debug message
 * @param *globalEnable - O - Enable debug messages
 *
 * @return  >=0 = PASS    <BR>
 *          <0 = FAIL  <BR>
 *
...
{
    hidMessageStruct msg;
    LCR_PrepReadCmd(DEBUG);

    if(LCR_Read() > 0)
    {
        memcpy(&msg, InputBuffer, 65);
        *enable = MAKE_WORD32(InputBuffer[7], InputBuffer[6], InputBuffer[5], InputBuffer[4]);
        *alloc = InputBuffer[8];
        *used = InputBuffer[9];
        *globalEnable = InputBuffer[10];
        return 0;
    }
    return -1;
}

int LCR_SetDebugMask(uint32 DebugMask, BOOL globalEnable)
/**
 * (I2C: 0x5B)
 * (USB: CMD2: 0x1A, CMD3: 0x5B)
 *
 * @param DebugMask - I - Each major sub-system is provided with 1-bit mask. User can enable or disable
 * @param globalEnable - I - Enable debug messages
 *
 * @return  >=0 = PASS    <BR>
 *          <0 = FAIL  <BR>
 *
 */
{
    ...
    msg.text.data[2] = DebugMask;
    msg.text.data[3] = DebugMask >> 8;
    msg.text.data[4] = DebugMask >> 16;
    msg.text.data[5] = DebugMask >> 24;
    msg.text.data[6] = globalEnable;
